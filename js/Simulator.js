  ...
  step(timestamp) {
    ...
    //
      ...
      this.dynamicObstacles.forEach(do_ => {
        if(new Date().getTime() - this.lastPrintTime > 1000 && Math.abs(do_.positionAtTime(timestamp).x - this.car.position.x) < 50 && Math.abs(do_.positionAtTime(timestamp).y - this.car.position.y) < 50)
          this.car.score = this.car.score - 5;
          this.lastPrintTime = new Date().getTime();
          console.log(this.car.score)
      });
